...
function local_birth_date(data) {
  ...
  presentations.forEach((p) => {
    ...
    const pais = p.country || "Perú";
    if (!groups[pais]) groups[pais] = [];
    groups[pais].push(p);
  });

  // Construir string final por país
  const countryStrings = [];
  for (const [pais, prods] of Object.entries(groups)) {
    const frases = prods.map((p, index) => {
      // El requirement pide: nombre comercial + forma farmaceutica
      const brand = p.brand_name || "Producto";
      // En la UI, p.presentation_name puede tener la concentración y forma (ej. "10mg + 10mg Tableta"), 
      // y pharma_form a veces es solo "Tableta". Usaremos presentation_name que suele ser más descriptivo.
      // Si la UI separa, "brand_name + presentation_name" nos da "EZTIM-S 10mg + 10mg Tableta".
      // Revisando el requerimiento: "EZTIM-S® 10mg + 10mg (Ezetimiba + Simvastatina) Tableta"
      // Asumiremos que presentation_name viene con eso.
      const desc = p.presentation_name || p.pharma_form || "";
      if (index === 0) {
        return `${pais} para ${brand} ${desc} corresponde al ${p.local_auth_date}`;
      } else {
        return `para ${brand} ${desc} corresponde al ${p.local_auth_date}`;
      }
    });
    countryStrings.push(joinNoAnd(frases));
  }

  // Juntar todo, ej: "Perú para X corresponde a Y, para Z corresponde a W, Colombia para A corresponde a B"
  return joinNoAnd(countryStrings);
}
...
//    2+ indicaciones -> lista con viñetas.
// =============================================================================
...
function indications_with_dose(data) {
  const { indicaciones_posologia } = data;

  if (!Array.isArray(indicaciones_posologia) || indicaciones_posologia.length === 0) {
    throw new Error("indications_with_dose: se requiere la lista de indicaciones_posologia.");
  }
  ...
  const frases = indicaciones_posologia.map((item, index) => {
    const ind = item.indicacion || "Indicación no especificada";
    const pos = item.posologia || "posología no especificada";
    return `${toRoman(index + 1)}) ${ind} a una dosis recomendada de ${pos}.`;
  });
